...
/*   Updated: 2024/11/27 10:57:04 by qhauuy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->rays_minimap = mlx_new_image(data->mlx, \
		data->minimap->width, data->minimap->height);
	if (data->rays_minimap == NULL)
		return (mlx_perror_exit(data));
	if (mlx_image_to_window(data->mlx, data->rays_minimap, \
		data->box_size, data->box_size) == -1)
	{
		mlx_delete_image(data->mlx, data->rays_minimap);
		return (mlx_perror_exit(data));
	}
