...
/*   Updated: 2024/11/30 21:07:31 by qhauuy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "main.h"

void	fill_maze_path(t_data *data)
{
	int	i;
	int	j;

	i = 0;
	while (i < MAZE_PATH_WIDTH)
	{
		j = 0;
		while (j < MAZE_PATH_HEIGHT)
		{
			data->maze_path[i][j].c = 'x';
			j++;
		}
		i++;
	}
	data->maze_path[MAZE_START_I][MAZE_START_J].c = '0';
}

void	create_maze_path(t_data *data)
{
	int	i;

	data->maze_path = ft_calloc(MAZE_PATH_WIDTH + 1, sizeof(t_maze_cell *));
	if (data->maze_path == NULL)
		error_exit(MALLOC, data);
	i = 0;
	while (i < MAZE_PATH_WIDTH)
	{
		data->maze_path[i] = ft_calloc(MAZE_PATH_HEIGHT + 1, sizeof(t_maze_cell));
		if (data->maze_path[i] == NULL)
			error_exit(MALLOC, data);
		i++;
	}
	fill_maze_path(data);
...
void	fill_maze(t_data *data)
{
	...
	data->maze[2 * MAZE_START_I + 1][2 * MAZE_START_J + 1] = 'N';
}
