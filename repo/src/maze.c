...
/*   Updated: 2024/11/30 18:01:19 by qhauuy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_maze(t_data *data)
{
	...
	data->maze = ft_calloc(MAZE_WIDTH + 1, sizeof(t_maze_cell *));
	if (data->maze == NULL)
		error_exit(MALLOC, data);
	i = 0;
	while (i < MAZE_WIDTH)
	{
		data->maze[i] = ft_calloc(MAZE_HEIGHT + 1, sizeof(t_maze_cell));
		if (data->maze[i] == NULL)
			error_exit(MALLOC, data);
		i++;
	}
